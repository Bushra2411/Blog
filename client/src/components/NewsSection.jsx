...
function SportsNewsCategory({ posts }) {
  const sportsPosts = posts.filter((post) => post.categories?.includes("sports"));
          ...
          {sportsPosts[0] && (
            <Link to={`/post/${sportsPosts[0]._id}`} style={{ textDecoration: "none", color: "inherit" }}>
              <div className="news-card big">
                <img src={`http://localhost:5000/images/${sportsPosts[0].photo}`} alt="Big News" />
          ...
          {sportsPosts[1] && (
            <Link to={`/post/${sportsPosts[1]._id}`} style={{ textDecoration: "none", color: "inherit" }}>
              <div className="news-card featured">
                <img src={`http://localhost:5000/images/${sportsPosts[1].photo}`} alt="Featured" />
          ...
          {sportsPosts.slice(2, 5).map((post) => (
            <Link to={`/post/${post._id}`} key={post._id} style={{ textDecoration: "none", color: "inherit" }}>
              <div className="news-card small">
                <img src={`http://localhost:5000/images/${post.photo}`} alt={post.title} />
                <p>{post.title}</p>
              </div>
            </Link>
          ))}
...
function BusinessNewsCategory({ posts }) {
  const businessPosts = posts.filter((post) => post.categories?.includes("business"));
  ...
  return (
    <div className="news business-news" id="businessNews">
      ...
      <div className="business-layout">
        {businessPosts[0] && (
          <Link to={`/post/${businessPosts[0]._id}`} className="featured-story">
            <img src={`http://localhost:5000/images/${businessPosts[0].photo}`} alt="featured story" />
        ...
        <div className="headline-grid">
          {businessPosts.slice(1, 5).map((post) => (
            <Link to={`/post/${post._id}`} key={post._id} className="post-link headline-card">
              <img src={`http://localhost:5000/images/${post.photo}`} alt={post.title} />
              <h3>{post.title}</h3>
            </Link>
          ))}
